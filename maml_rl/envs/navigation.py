...
class Navigation2DEnv(gym.Env):
    ...
    def sample_tasks(self, unseen_tasks, num_tasks):
        goals = self.np_random.uniform(self.min, self.max, size=(num_tasks, 2))
        unseen = [goal['goal'] for goal in unseen_tasks]
        for i in range(goals.shape[0]):
            while (goals[i] == unseen).all(1).any():
                goals[i] = self.np_random.uniform(self.min, self.max, 2)
