...
class MetaLearner(object):
    ...
    def inner_loss(self,task, episodes, params=None):
    ...
    def adapt(self, task, episodes, first_order=False):
        ...
        loss = self.inner_loss(task, episodes)
    ...
    def sample(self, tasks, scale, first_order=False):
        ...
        for task in tasks:
            ...
            params = self.adapt(task_tensor, train_episodes, first_order=first_order)

            valid_episodes = self.sampler.sample(task_tensor, self.policy, params=params,
                gamma=self.gamma, device=self.device)
            episodes.append((task_tensor, train_episodes, valid_episodes))
        return episodes
    ...
    def evaluate_task(self, unseen_task, scale, episode_num, gradient_steps=1):
