...
class BatchSampler(object):
    ...
    def sample(self,task, policy, params=None, gamma=0.95, device='cpu'):
        ...
        while (not all(dones)) or (not self.queue.empty()):
            with torch.no_grad():
                ...
                actions_tensor = policy(observations_tensor,task, params=params).sample()
    ...
    def get_task_tensor(self, task, scale):
        ...
        if 'velocity' in task.keys():
            ...
            task_x, task_y = task['direction'][0] / max_, task['direction'][1]
